...
###############################################################
submission<-read.csv("submissionTN_Net.csv")
submission<-read.csv("nnsub.csv")
submission<-read.csv("sub2021.csv")
submission<-read.csv("sub2021_no_Massey.csv")
submission<-read.csv("sub2021_round_2_XGBoost.csv")
submission<-read.csv("C:/Users/tnovi/Documents/R/kaggle_mania_2021_Men/submissionTN_Net_Stage1.csv")


submission<-read.csv("C:/Users/tnovi/Documents/R/kaggle_mania_2021_Men/submissionTN_Net_Stage_2_4_hidden.csv")
submission<-read.csv("sub2021_KPom_Qual_Eff.csv")
head(submission)
...
#seeds<-seeds[Season == 2018,]
seeds<-seeds[Season == 2021,]

pdf("seedbracket_2021_NN.pdf",width=11,height=8.5)
tgtseason<-"2021"
...
submission<-merge(submission,seeds, by.x = c("season","team"),by.y=c("Season","TeamID"))
names(submission)<-c("season", "team1", "id", "pred1", "region1", "seed1")
names(teams)<-c("team1", "name1")
submission<-merge(submission,teams, by.x = c("team1"), by.y= c("team1"))
submission$team2<-as.numeric(substr(submission[,3],11,14))
names(seeds)<-c("Season","team2","region2","seed2")
submission<-merge(submission,seeds, by.x = c("season","team2"),by.y = c("Season","team2"))
names(teams)<-c("team2", "name2")
submission<-merge(submission,teams, by.x = c("team2"), by.y= c("team2"))
